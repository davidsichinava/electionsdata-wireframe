#!/usr/bin/env node
// Ingest the Elected sheet from src/data/raw/adjara_2008_results.xlsx into
// src/data/candidates/adj_2008_elected.csv.
//
// The PR and SMD candidate rosters for adj_2008 already exist (sourced from
// adjara_2008_candidates.xlsx earlier); this script only adds the winners.
//
// Usage:
//   node scripts/ingest-adj2008.js           (dry-run → src/data/candidates/_adj2008_ingest/)
//   node scripts/ingest-adj2008.js --apply   (writes directly to src/data/candidates/)
...
const RAW_XLSX = path.join(SRC, "data/raw/adjara_2008_results.xlsx");

const APPLY = process.argv.includes("--apply");
const OUT_DIR = APPLY ? CAND_DIR : path.join(CAND_DIR, "_adj2008_ingest");
...
const adj2008Doc = yaml.load(
  fs.readFileSync(path.join(ELECTIONS_DIR, "adjara/adj_2008.yml"), "utf8")
);
...
function resolvePartyId(label) {
  ...
  const cands1 = [];
  for (const p of (adj2008Doc?.parties ?? [])) {
...
const SOURCE = "adjara_2008_results.xlsx";

// ─── Map to canonical ──────────────────────────────────────────────────────

const elOut = [];
for (const r of electedRows) {
  const sys = (r.system ?? "").toLowerCase();
  const voteType = sys === "pr" ? "pr" : "smd";
  ...
  elOut.push({
    ...
    party_code: "",
    district_id: voteType === "smd" ? r.district_code : "",
    district_name_ka: voteType === "smd" ? r.district_name : "",
    list_order: "",
    ballot_number: "",
...
writeCanonicalCsv("adj_2008_elected.csv", elOut);
