...
//
//   src/data/candidates/parl_2020_pr.csv
//   src/data/candidates/parl_2020_smd.csv
//   src/data/candidates/parl_2020_elected.csv
//
// Usage:
//   node scripts/ingest-parl2020.js          (dry-run → src/data/candidates/_2020_ingest/)
//   node scripts/ingest-parl2020.js --apply  (writes directly to src/data/candidates/)
...
const RAW_XLSX = path.join(SRC, "data/raw/party_lists_2020_georgia_unified.xlsx");

const APPLY = process.argv.includes("--apply");
const OUT_DIR = APPLY ? CAND_DIR : path.join(CAND_DIR, "_2020_ingest");
...
// ─── Party-id resolver (mirrors scripts/ingest-parl2016.js) ────────────────

const partiesYml = yaml.load(fs.readFileSync(PARTIES_YML, "utf8"));
const partyRegistry = {};
for (const p of (partiesYml?.parties ?? [])) partyRegistry[p.id] = p;

const parl2020Doc = yaml.load(
  fs.readFileSync(path.join(ELECTIONS_DIR, "parliamentary/parl_2020.yml"), "utf8")
);
...
function resolvePartyId(label) {
  ...
  const cands1 = [];
  for (const p of (parl2020Doc?.parties ?? [])) {
...
const SOURCE = "party_lists_2020_georgia_unified.xlsx";

// ─── PR list candidates ────────────────────────────────────────────────────

const prOut = [];
for (const r of prRows) {
  const partyLabel = r["Party Name"];
  ...
  prOut.push({
    ...
    party_code: r["Party Number"],
    district_id: "",
    district_name_ka: "",
    list_order: r["Order ID"],
    ballot_number: "",
    first_name: r["Name"],
    last_name: r["Last Name"],
    name_ka: [r["Name"], r["Last Name"]].filter(Boolean).join(" "),
...
for (const r of smdRows) {
  const partyLabel = r["Endorsing party"];
  ...
  smdOut.push({
    ...
    district_name_ka: "",
    list_order: "",
    ballot_number: r["Candidate number"],
    first_name: r["Name"],
    last_name: r["Last Name"],
    name_ka: [r["Name"], r["Last Name"]].filter(Boolean).join(" "),
    partisanship: "",
...
writeCanonicalCsv("parl_2020_pr.csv", prOut);
writeCanonicalCsv("parl_2020_smd.csv", smdOut);
writeCanonicalCsv("parl_2020_elected.csv", elOut);
