...
async function generateBundle(election, sub, generatedAt) {
  ...
  if (isMain) {
    ...
    // Canonical candidate sheets (sourced from adg_2025_candidates_unified.xlsx).
...
export async function generateLocal2025Downloads({ generatedAt = new Date() } = {}) {
  const election = readElection("local_2025");
  const results = [];
